...
class SexModel(nn.Module):
    ...
    `Demo notebook <https://github.com/mlmed/radiology_engine/blob/main/scripts/sex_prediction.ipynb>`__
    ...
    .. code-block:: python
        ...
        model = re.baseline_models.mira.SexModel()

        image = re.utils.load_image('00027426_000.png')
    ...
    def __init__(self, weights=True):
        ...
        if weights:
            ...
            url = 'https://github.com/mlmed/radiology_engine/releases/download/v1/mira_sex_resnet-all_epoch_13-step_7125.ckpt'

            weights_filename = "mira_sex_resnet-all_epoch_13-step_7125.ckpt"
            weights_storage_folder = os.path.expanduser(os.path.join("~", ".radiology_engine", "models_data"))
            ...
            if not os.path.isfile(self.weights_filename_local):
                ...
                try:
                    re.utils.download(url, self.weights_filename_local)
    ...
    def forward(self, x):
        ...
        # Convert from radiology_engine range [-1024, 1024] to [0, 1] 
        x = (x + 1024) / 2048
